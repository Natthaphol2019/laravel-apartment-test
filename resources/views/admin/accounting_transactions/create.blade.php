...
@section('content')
    ...
    <div class="container-fluid py-4" style="min-height: 100vh; background-color: {{ $bgLight }};">
        <form action="{{ route('admin.accounting_transactions.store') }}" method="POST">
            @csrf
                ...
                <div class="col-xl-11">
                    <div class="card shadow-sm border-0 mb-4">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered align-middle" id="itemsTable">
                                    {{-- ส่วนหัวตารางที่ปรับปรุงใหม่ --}}
                                    <thead class="table-{{ $themeColor }} text-center small">
                                        <tr>
                                            <th width="15%">หมวดหมู่</th>
                                            <th width="20%">หัวข้อรายการธุรกรรม</th> {{-- เปลี่ยนชื่อตามที่ขอ --}}
                                            <th width="25%">รายละเอียดเพิ่มเติม (ถ้ามี)</th> {{-- เพิ่มคอลัมน์ใหม่ --}}
                                            <th width="12%">เลขห้อง (ถ้ามี)</th>
                                            <th width="23%">จำนวนเงิน (บาท)</th>
                                            <th width="5%">ลบ</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr class="item-row">
                                            <td>
                                                <select name="items[0][category_id]"
                                                    class="form-select border-{{ $themeColor }}" required>
                                            ...
                                            <td>
                                                <input type="text" name="items[0][title]" class="form-control"
                                                    placeholder="เช่น ค่าส่วนกลาง" required>
                                            </td>
                                            <td>
                                                {{-- ✅ เพิ่มช่องบันทึก Description --}}
                                                <textarea name="items[0][description]" class="form-control" rows="1" placeholder="ระบุรายละเอียดเพิ่มเติม..."></textarea>
                                            </td>
                                            <td>
                                                <input type="text" name="items[0][room_number]"
                                                    class="form-control text-center" list="roomList"
                                                    placeholder="พิมพ์เลขห้อง...">
                                            </td>
                                            <td>
                                                <div class="input-group">
                                                    <span
                                                        class="input-group-text bg-white text-{{ $themeColor }} fw-bold">{{ $isIncome ? '+' : '-' }}</span>
                                                    <input type="number" name="items[0][amount]"
                                                        class="form-control text-end fw-bold amount-input" step="0.01"
                                                        min="0" oninput="calculateTotal()" required>
                                                </div>
                                            </td>
                                            <td class="text-center">
                                                <button type="button"
                                                    class="btn btn-{{ $themeColor }} btn-sm rounded-circle shadow-sm"
                            ...
                            {{-- Datalist สำหรับห้องพัก --}}
                            <datalist id="roomList">
                                @foreach ($tenants as $t)
                                    <option value="{{ $t->room->room_number }}">
                                @endforeach
                            </datalist>

                            <div class="mt-3">
                                <button type="button" class="btn btn-outline-{{ $themeColor }} btn-sm px-4 fw-bold"
                                    onclick="addRow()">
                        ...
                        <div class="card-footer bg-light p-4 text-end border-0">
                            <button type="button" class="btn btn-{{ $themeColor }} btn-lg px-5 shadow-sm fw-bold"
                                onclick="confirmSave()">
...
@push('scripts')
    <script>
        ...
        function addRow() {
            ...
            const isIncome = {{ $isIncome ? 'true' : 'false' }};
            const sign = isIncome ? '+' : '-';
            ...
            const html = `
                    <tr class="item-row">
                        ...
                        <td>
                            <input type="text" name="items[${rowIdx}][room_number]" class="form-control text-center" 
                                list="roomList" placeholder="พิมพ์เลขห้อง...">
                        </td>
                        <td>
                            <div class="input-group">
                                <span class="input-group-text bg-white text-${themeColor} fw-bold">${sign}</span>
                                <input type="number" name="items[${rowIdx}][amount]" class="form-control text-end fw-bold amount-input" 
        ...
        function calculateTotal() {
            ...
            document.getElementById('grandTotal').innerText = total.toLocaleString('th-TH', {
    ...
    // 1. ดึงแถวทั้งหมดที่มีคลาส .item-row
    ...
    // ตรวจสอบวันที่ทำรายการก่อนเป็นอันดับแรก
    if (!dateInput) {
        Swal.fire({ icon: 'warning', title: 'กรุณาระบุวันที่', text: 'ต้องเลือกวันที่ทำรายการก่อนบันทึก' });
        return;
    }

    // 2. วนลูปตรวจสอบทุกแถวแบบละเอียด
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        // ใช้ Selectors ที่เจาะจงภายในแถวนั้นๆ (row.querySelector)
        ...
        // เช็คว่ามีค่าว่าง หรือจำนวนเงินเป็น 0 หรือไม่
        if (!category || !title || !amount || parseFloat(amount) <= 0) {
            ...
            break; // หยุดเช็คทันทีเมื่อเจอแถวที่ผิด
        }
    }

    // 3. หากมีแถวไหนไม่ผ่าน ให้หยุดและแจ้งเตือน
    if (!isValid) {
        Swal.fire({
            icon: 'error',
    ...
    // 4. ถ้าผ่านหมดแล้ว เตรียมข้อมูลวันที่ไทยเพื่อแสดงในผลสรุป
    ...
    // 5. แสดง SweetAlert ยืนยันขั้นตอนสุดท้าย
    Swal.fire({
