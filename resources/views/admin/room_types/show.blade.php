...
<div class="modal fade" id="insertRoomTypeModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <form action="{{ route('admin.room_types.insert') }}" method="POST">
                @csrf
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label for="name" class="form-label fw-bold">ชื่อประเภทห้อง</label>
                        <input type="text" name="name" class="form-control" placeholder="เช่น ห้องแอร์พร้อมเฟอร์นิเจอร์" required>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        // SweetAlert ยืนยันการแก้ไข
        function confirmUpdate() {
            Swal.fire({
                title: 'ยืนยันการแก้ไข?',
                text: "คุณต้องการบันทึกการเปลี่ยนแปลงข้อมูลประเภทห้องใช่หรือไม่",
                icon: 'question',
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                text: "หากลบแล้วข้อมูลประเภทห้องนี้จะไม่สามารถกู้คืนได้!",
                icon: 'warning',
                showCancelButton: true,
